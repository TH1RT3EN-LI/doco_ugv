...
namespace lslidar_driver
{
	...
	bool LslidarDriver::loadParameters()
	{
		...
		frame_id = std::string("ugv_laser");
		scan_topic = std::string("/ugv/scan");
		lidar_name = std::string("M10");
		pointcloud_topic = std::string("/ugv/lidar/points");
		is_start = true;
		min_range = 0.3;
		max_range = 100.0;
		use_gps_ts = true;
		...
		this->declare_parameter<std::string>("frame_id", "ugv_laser");
		this->declare_parameter<std::string>("scan_topic", "/ugv/scan");
		this->declare_parameter<std::string>("pointcloud_topic", "/ugv/lidar/points");
		this->declare_parameter<int>("fixed_scan_count", 0);
		this->declare_parameter<int>("scan_count_warmup", 5);
		...
		this->get_parameter("fixed_scan_count", fixed_scan_count_);
		this->get_parameter("scan_count_warmup", scan_count_warmup_);
		...
		if (fixed_scan_count_ < 0)
			fixed_scan_count_ = 0;
		if (scan_count_warmup_ < 1)
			scan_count_warmup_ = 1;
		...
		// Initialize timestamps to avoid a huge first scan_time/time_increment when the first sweep arrives.
		time_ = get_clock()->now();
		pre_time_ = time_;
	...
	int LslidarDriver::getScan(std::vector<ScanPoint> &points, rclcpp::Time &scan_time, float &scan_duration, int &count)
	{
		boost::unique_lock<boost::mutex> lock(mutex_);
		points.assign(scan_points_bak_.begin(), scan_points_bak_.end());
		count = count_num_bak_;
		scan_time = pre_time_;
		...
		while (rclcpp::ok())
		{
			...
			if (lidar_name == "N10_P" || lidar_name == "M10_DOUBLE")
			{
				if (pubScan)
				{
					...
					float scan_time;
					int count = 0;
					this->getScan(points, start_time, scan_time, count);
					const int scan_num = count;
					if (scan_num < 2)
						continue;
					int output_scan_num = scan_num;
					bool publish_scan = true;
					if (fixed_scan_count_ > 0)
					{
						output_scan_num = fixed_scan_count_;
						if (output_scan_num < scan_num)
						{
							RCLCPP_WARN(this->get_logger(),
										"fixed_scan_count (%d) < current scan size (%d); using %d to avoid dropping points",
										output_scan_num, scan_num, scan_num);
							output_scan_num = scan_num;
						}
						expected_scan_count_ = output_scan_num;
						scan_count_locked_ = true;
					}
					else if (!scan_count_locked_)
					{
						if (scan_num > max_scan_count_)
							max_scan_count_ = scan_num;
						scan_count_samples_++;
						if (scan_count_samples_ >= scan_count_warmup_)
						{
							expected_scan_count_ = max_scan_count_;
							scan_count_locked_ = true;
							RCLCPP_INFO(this->get_logger(), "Locking LaserScan size to %d points", expected_scan_count_);
						}
						else
						{
							publish_scan = false;
					...
					if (scan_count_locked_)
						output_scan_num = expected_scan_count_;
					...
					if (publish_scan)
					{
						...
						{
							// Use the scan acquisition time instead of "now" to avoid TF extrapolation / jitter,
							// especially noticeable when the robot moves fast.
							const double dur_s = std::max(0.0, static_cast<double>(scan_time));
							scan->header.stamp = start_time + rclcpp::Duration::from_seconds(dur_s);
						}

						// Publish a self-consistent 2D scan (one range per angle bin).
						// N10_P provides two returns per angle (stored at i and i+3000); we merge them by taking
						// the nearer valid return (0.0 means invalid/out-of-range).
						scan->angle_min = 0.0;
						scan->angle_increment = 2 * M_PI / static_cast<double>(output_scan_num);
						scan->angle_max = scan->angle_min + scan->angle_increment * static_cast<double>(output_scan_num - 1);
						scan->range_min = min_range;
						scan->range_max = max_range;
						scan->ranges.reserve(output_scan_num);
						scan->ranges.assign(output_scan_num, std::numeric_limits<float>::infinity());
						scan->intensities.reserve(output_scan_num);
						scan->intensities.assign(output_scan_num, 0.0f);
						scan->scan_time = scan_time;
						scan->time_increment = scan_time / static_cast<double>(output_scan_num);

						for (int i = 0; i < scan_num && i < static_cast<int>(points.size()); i++)
						{
							// Map [0, 360) degrees into [0, scan_num) bins; use floor for stable binning.
							int point_idx = static_cast<int>(floor((360.0 - points[i].degree) * output_scan_num / 360.0));
							point_idx %= output_scan_num;
							if (point_idx < 0)
								point_idx += output_scan_num;

							const int echo_idx = i + 3000;
							const bool echo_ok = (echo_idx >= 0 && echo_idx < static_cast<int>(points.size()));

							const double r1 = points[i].range;
							const double r2 = echo_ok ? points[echo_idx].range : 0.0;
							const bool v1 = (r1 > 0.0);
							const bool v2 = (r2 > 0.0);
							...
							if (v1 && v2)
							{
								...
								{
									best_r = r1;
									best_intensity = points[i].intensity;
								}
								else
								{
									best_r = r2;
									best_intensity = points[echo_idx].intensity;
								}
							}
							else if (v1)
							{
							...
							float &slot = scan->ranges[point_idx];
							if (!std::isfinite(slot) || best_r < slot)
							{
								slot = static_cast<float>(best_r);
								scan->intensities[point_idx] = static_cast<float>(best_intensity);
							}
						}
						...
						if (!pass_point)
							{
								...
								VPoint point;
								int point_idx = round(degree * count / 360);
								point.timestamp = timestamp - point_idx * (scan_time / count);
								// printf("timestamp = %f\n",point.timestamp);
			...
			else
			{
					if (pubScan)
					{
						...
						int count = 0;
						this->getScan(points, start_time, scan_time, count);
						int scan_num = ceil((angle_able_max - angle_able_min) / 360 * count) + 1;
						if (scan_num < 2)
							continue;
						int output_scan_num = scan_num;
						bool publish_scan = true;
						...
						else
						{
							...
							// Keep angle metadata consistent with the actual published array length.
							scan->angle_increment = (scan->angle_max - scan->angle_min) / static_cast<double>(output_scan_num - 1);
						scan->range_min = min_range;
						scan->range_max = max_range;
						scan->ranges.reserve(output_scan_num);
						scan->ranges.assign(output_scan_num, std::numeric_limits<float>::infinity());
						scan->intensities.reserve(output_scan_num);
						scan->intensities.assign(output_scan_num, std::numeric_limits<float>::infinity());
						scan->scan_time = scan_time;
						scan->time_increment = scan_time / static_cast<double>(output_scan_num - 1);
						...
						for (int i = 0; i < count && i < static_cast<int>(points.size()); i++)
						{
							int point_idx = round((360 - points[i].degree) * count / 360);
							if (point_idx < (end_num - count))
								point_idx += count;
							point_idx = point_idx - start_num;
							if (point_idx < 0 || point_idx >= output_scan_num)
								continue;
