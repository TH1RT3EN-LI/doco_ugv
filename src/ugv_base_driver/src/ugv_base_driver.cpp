...
public:
        ...
        : Node("ugv_base_driver")
    {
        ...
        // -------- publishers --------
        voltage_pub_ = this->create_publisher<std_msgs::msg::Float32>("/ugv/battery_voltage", 10);
        odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("/ugv/odom", 50);
        imu_pub_ = this->create_publisher<sensor_msgs::msg::Imu>("/ugv/imu", 20);
    ...
    ~Duojin01BaseDriverNode() override
    {
        ...
        RCLCPP_INFO(this->get_logger(), "ugv_base_driver node shutting down");
    }
...
private:
    ...
    void on_timer()
    {
        ...
        if (serial_ && serial_->is_open())
        {
            ...
            const int64_t last_rx_ns = last_serial_rx_steady_ns_.load(std::memory_order_acquire);
            const double since_last = static_cast<double>(now_ns - last_rx_ns) * 1e-9;
...
private:
    ...
    std::atomic<int64_t> last_serial_rx_steady_ns_{0};
