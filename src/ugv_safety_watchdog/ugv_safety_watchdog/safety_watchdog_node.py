...
class SafetyWatchdogNode(Node):
    ...
    1. 电池电压过低
    2. IMU倾斜
    3. /ugv/odom, /ugv/imu, /ugv/battery_voltage 话题超时
    """

    REASON_ODOM_TIMEOUT = "底盘里程计 (/ugv/odom) 超时，疑似串口断联"
    REASON_IMU_TIMEOUT = "IMU (/ugv/imu) 超时，疑似串口断联"
    REASON_BATTERY_TIMEOUT = "电池电压 (/ugv/battery_voltage) 超时，疑似串口断联"
    ...
    def __init__(self):
        ...
        self._lock_pub = self.create_publisher(Bool, "/ugv/watchdog/lock", lock_qos)


        self._status_pub = self.create_publisher(String, "/ugv/watchdog/status", 10)
        ...
        self.create_subscription(Imu, "/ugv/imu", self._imu_cb, 10)
        self.create_subscription(Float32, "/ugv/battery_voltage", self._battery_cb, 10)

        self.create_service(Trigger, "/ugv/watchdog/reset", self._reset_cb)
        ...
        if self._triggered:
            ...
            if since_reminder >= 5.0:
                ...
                self.get_logger().warn(
                    ...
                    f"| 恢复: ros2 service call /ugv/watchdog/reset std_srvs/srv/Trigger")
            return
    ...
    def _trigger_estop(self, reason: str):
        ...
        self.get_logger().error("!  手动恢复: ros2 service call /ugv/watchdog/reset std_srvs/srv/Trigger")
